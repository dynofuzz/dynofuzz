...
\hline
        & SMT              & Gen.     & Eval.      & Save        & Total  \\
\hline
"""
    template = r"""
Avgerage   & {avg_tsmt:.0f} &  {avg_tgen:.0f} & {avg_trun:.0f} & {avg_tsave:.0f} & {avg_tsum:.0f} \\
P50        & {p50_tsmt:.0f} &  {p50_tgen:.0f} & {p50_trun:.0f} & {p50_tsave:.0f} & {p50_tsum:.0f} \\
P90        & {p90_tsmt:.0f} &  {p90_tgen:.0f} & {p90_trun:.0f} & {p90_tsave:.0f} & {p90_tsum:.0f} \\
P99        & {p99_tsmt:.0f} &  {p99_tgen:.0f} & {p99_trun:.0f} & {p99_tsave:.0f} & {p99_tsum:.0f} \\
Percentage & {psmt:.0f}\%   &  {pgen:.0f}\%   & {prun:.0f}\%   & {psave:.0f}\% & 100\% \\
"""
    suffix = r"""
\hline
\end{tabular}
