...
from autoinf.inference.utils import str_equivalent, wrap_time
...
class TreeDatabase:
    def __init__(
        ...
        if from_DB:
            ...
            if DB_Name == None:
                ...
                DB_Name = f"{InformationRootDir}/tree-{argCount}{maxHeight}{suffix}.pkl"
    ...
    def dump(self, DB_Name=None):
        ...
        treeMode = "" if EnableDiv else "-nodiv"
        if DB_Name == None:
            DB_Name = f"{InformationRootDir}/tree-{self.argCount}{self.maxHeight}{treeMode}.pkl"
...
def TreeGeneration(ArgSet, depth: int):
    """
...
@wrap_time
def build_tree(depth=TreeMaxHeight, ArgCount=TreeArgCount):
    global TreeDB
    TreeDB = TreeDatabase(False, depth, ArgCount)
    generate_testcase(TreeArgCount)
    NodeGeneration(ArgCount)
    for dep in range(1, depth + 1):
        for ArgSet in range(0, (1 << ArgCount)):
            TreeGeneration(ArgSet, dep)
    TreeDB.strip()
    TreeDB.dump()
