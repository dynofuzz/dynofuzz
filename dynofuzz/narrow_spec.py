...
from dynofuzz import __version__
...
from dynofuzz.gir import GraphIR, InstExpr
from dynofuzz.logging import DTEST_LOG
from dynofuzz.materialize import Model, TestCase

DYNOFUZZ_CACHE_DIR = user_cache_dir(f"dynofuzz-{__version__}")
...
def auto_opconfig(
    ...
    cache_path = os.path.join(
        DYNOFUZZ_CACHE_DIR, get_cache_name(model_cls, factory) + ".yaml"
    )
    # mkdir -p NNSMITH_CACHE_DIR
    if not os.path.exists(DYNOFUZZ_CACHE_DIR):
        os.makedirs(DYNOFUZZ_CACHE_DIR)
